...
void MagAngle::ReadMag1Angle() 
	{
		...
		u8_error_lsb = received_data[1] >> 6;
		u8_crc_dec = received_data[7];
		u8_virtualgain_dec = received_data[4];
		u8_rollcnt_dec = received_data[6] & 0x3F;
		Param::SetInt(Param::Angle_LSB, u16_angle_lsb);
		Param::SetInt(Param::Angle, f32_angle_degrees);
		Param::SetInt(Param::ERR_Bits, u8_error_lsb);
